...
import { adminUpdateBlog } from "@/features/admin/api/admin.api";
...
export default function EditBlogModal({ isOpen, blog, onClose, onSuccess }: Props) {
  ...
  useEffect(() => {
    ...
    setImagePreview(isValidUrl(blog.image) ? blog.image : null);
    setImageFile(null);
  }, [blog]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await adminUpdateBlog(blog.id, formData);
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 sticky top-0 bg-white z-10">
          <div>
            <h2 className="text-lg font-bold text-gray-800">Edit Blog</h2>
            <p className="text-xs text-gray-400 mt-0.5">Update your blog post</p>
          </div>
              ...
              <div>
                ...
                {imageFile && (
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <button onClick={() => { setImageFile(null); setImagePreview(isValidUrl(blog?.image) ? blog!.image : null); }}>
